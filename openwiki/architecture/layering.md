...
That is not tidiness. It is the reason 476 tests run in under a second with no Raycast runtime,
no network, no keychain and no clock. Every decision worth testing lives in `lib`, and
...
| [`auth/keychain.ts`](../../src/lib/auth/keychain.ts) | an `Exec` function |
